...
function findAdults(persons) {
  const adultArray = persons.filter((person) => person.age >= 18);
  const adultWomen = adultArray.filter((person) => person.sex === "female");
  const adultMen = adultArray.filter((person) => person.sex === "male");
